function fireTarget = chooseTargeti (targetList, num, myFleetAns, opFleetAns,state)
%%  chooseTargeti 获取数个靶舰位置函数，靶舰可重复，测试完毕
...
opTrautskill_all = ["迷之嘲讽","希望的曙光","盛开的迎风花"];
opTrautprocent_all = [0.22, 0.3 , 0.3];
...
for k = 1 : num
    sum = 0;
    for i = length( targetList )
       sum = sum + trautRate( i );
    end

    index = sum * rand(1);
    flag = 0;
    for i = length( targetList ) + 1
